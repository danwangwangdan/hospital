...
Page({
    ...
    onLoad: function(options) {
        ...
        if (!that.data.isLogin) {
          ...
          //获取二级类型
          wx.request({
            url: app.globalData.localApiUrl + '/common/secTypes?firTypeId=' +0,
            method: 'GET',
            header: {
              'content-type': 'application/json'
            },
    ...
    firTypeChange: function(e) {
        ...
        if (this.data.secTypes[e.detail.choosedData[0]] != '其他问题') {
            this.setData({
                isAllOther: false
            });
        } else {
    ...
    submitTrouble: function(e) {
        ...
        if (content != "" || that.data.fileList.length != 0) {
            isBlankContent = false;
        }
        // console.log("fileList:" + (that.data.fileList.length != 0));
        // console.log("content:" + content);
        // console.log("true:" + that.data.isAllOther && isBlankContent);
        if (troubleOwner == "") {
            wx.showToast({
                title: '故障人不能为空',
                icon: 'none',
